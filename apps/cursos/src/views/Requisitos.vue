...
<template>
    <Card class="bg-white pt-8 pb-2 px-2 lg:px-8">
        <h1 class="text-2xl text-gray-500 font-semibold">Requisitos</h1>
        <div v-if="auth.checkPermission('crear requisito')" class="flex justify-center">
            <Tool-Tip message="Crear nuevo requisito" class="-mt-8 text-color-4">
                <Button @click="store.modal.new = true" icon="fas fa-plus" class="btn-primary" />
            </Tool-Tip>
        </div>
        <Data-Table v-if="auth.checkPermission('ver requisitos')" :headers="store.headers" :data="store.requisitos" :loading="store.loading.fetch" :export="auth.checkPermission('exportar requisitos')">
                ...
                <div class="relative">
                    <Drop-Down-Button icon="fas fa-ellipsis-vertical">
                        <ul class="grid gap-3 text-violet-500">
                            <li v-if="auth.checkPermission('editar requisito')" @click="store.edit(item)" class="hover:font-medium cursor-pointer">
                                Editar
                            </li>
                            <hr>
                            <template v-if="auth.checkPermission('eliminar requisito')">
    ...
    <!-- AREA DE MODALES -->
    <Modal :open="store.modal.new" icon="fas fa-list-check" title="Crear nuevo requisito">
        <template #close>
            <Icon @click="store.resetData" icon="fas fa-xmark" class="text-violet-200 text-xl hover:scale-125 cursor-pointer" />
        </template>
        <div class="grid gap-3">
            <Input v-model="store.requisito.nombre" option="label" title="Nombre requisito" :error="store.errors.hasOwnProperty('nombre')" />
        </div>
    ...
    <Modal :open="store.modal.edit" title="Editar requisito" icon="fas fa-list-check">
        <template #close>
            <Icon @click="store.resetData" icon="fas fa-xmark" class="text-violet-200 text-xl hover:scale-125 cursor-pointer" />
        </template>
        <div class="grid gap-3">
            <div v-if="store.requisito.estatus === 'I'" class="flex justify-end">
                <div class="flex gap-3 text-gray-500">
                    <span>Activo</span>
                    <Switch :values="['I','A']" v-model="store.requisito.estatus" class="bg-gray-400 w-12 has-[:checked]:bg-color-4"/>
                    <span>Inactivo</span>
                </div>
            </div>
            <Input v-model="store.requisito.nombre" option="label" title="Nombre requisito" :error="store.errors.hasOwnProperty('nombre')" />
    ...
    <Modal :open="store.modal.delete">
        <div class="flex items-center gap-4">
            ...
            <div class="grid">
                <h1>¿Esta seguro de eliminar el requisito?</h1>
                <h2 class="text-center font-semibold text-xl">{{ store.requisito.nombre }}</h2>
            </div>
        </div>
        <template #footer>
            <Button @click="store.resetData" text="Cancelar" icon="fas fa-xmark" class="btn-secondary" />
