...
export const useHorariosStore = defineStore('horarios', () => {
    ...
    const headers = [
        { title : 'id', key : 'id_horario', type : 'numeric' },
    ...
    async function show (id_horario) {
        loading.value.show = true
        try {
            const response = await axios.get('horarios/'+id_horario)
            horarios.value = response.data
        } catch (error) {
            if(error.response.data.hasOwnProperty('message') && error.response.data.hasOwnProperty('line')) {
                global.setAlert(error.response.data.message,'danger')
    ...
    async function update() {
        ...
        horario.value.dias = dias.value
        try {
            const response = await axios.put('horarios/' + horario.value.id_horario, horario.value)
            fetch()
            global.setAlert(response.data,'success')
            resetData()
        } catch (error) {
    ...
    async function destroy() {
        ...
        try {
            const response = await axios.delete('horarios/' + horario.value.id_horario)
            fetch()
            global.setAlert(response.data,'success')
            resetData()
        } catch (error) {
