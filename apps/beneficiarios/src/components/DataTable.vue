    ...
    <section class="px-4 lg:px-7">
        ...
        <div class="md:flex md:items-center md:justify-between">
            ...
            <div class="flex items-center gap-1">
                ...
                <Input icon="fas fa-search" type="search" placeholder="Buscar ..." v-model="search" />

                <div class="flex gap-1">
                    <!-- FILTER ADVANCE -->
                    <div class="relative" ref="target">
                        <Tool-Tip message="Filtro avanzado" class="-mt-7 text-color-4" v-if="props.filterAdvance">
                            <Button @click="openFilterOptions = !openFilterOptions" icon="fas fa-filter" class="btn-dark" :class="filters.length > 0 && filters[0].value != '' ? 'bg-green-500' : 'bg-gray-400'" />
                        </Tool-Tip>
                            <div v-show="openFilterOptions" class="bg-white border-2 rounded absolute mt-2 right-0 p-2 z-10">
                                <h1 class="font-semibold text-color-4 text-center text-xs whitespace-nowrap pb-1">
                                    Filtros avanzados
                                </h1>
                                <hr>
                    ...
                    <!-- END FILTER ADVANCE -->
                    <Tool-Tip message="Descargar" class="-mt-7 text-color-4">
                        <Button v-if="props.export && data.length > 0" @click="exportData" :loading="loadingExportData" icon="fas fa-file-excel" class="btn-success" />
                    </Tool-Tip>
                </div>
            </div>
        </div>
